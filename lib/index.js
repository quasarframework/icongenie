...
const fileSystem = {
  ...
  /**
   * @param  {String | Number | Buffer} file the file or stream to be read
   * @returns {Promise} a promise who will successed resolve with the context of the readed file
   */
  readFile: util.promisify(fs.readFile),
  /**
   * ensured than a given directory exists
   * @param  {string} path the path to the directory than be ensured that exists
...
let iconfactory = exports.iconfactory = {
   ...
   * @param {string} target - where to drop the images
   * @param {string} options - js object that defines path and sizes
   * @param {boolean} preventSubFolder
   */
  splash: async function(src, target, options, preventSubFolder) {
    let output
  ...
  svgDuochrome: async function(src, target, dest, preventSubFolder) {
    if (!dest) dest = 'extras'
    const path = preventSubFolder ? `${target}` : `${target}/${dest}`
    try {
      const traceParams = {
      ...
      let svg = await potraceExtra.posterize({ data, traceParams })
