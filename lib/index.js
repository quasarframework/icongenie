...
let iconfactory = exports.iconfactory = {
	...
	cordova: function (src, target, strategy) {
		...
		chain = chain.then(() => buildify(this.build(src, target, options)))
		chain = chain.then(() => buildify(strategy ? this.minify(target, settings.options.cordova, strategy, 'batch') : console.log('no minify strategy')))
		return chain
	},
	electron: function (src, target, strategy) {
		...
		chain = chain.then(() => buildify(this.build(src, target, options)))
		chain = chain.then(() => buildify(strategy ? this.minify(target, settings.options.electron, strategy, 'batch') : console.log('no minify strategy')))
	...
	pwa: function (src, target, strategy) {
		...
		chain = chain.then(() => verify(src, target))
		chain = chain.then(() => buildify(this.build(src, target, options)))
		chain = chain.then(() => buildify(strategy ? this.minify(target, settings.options.pwa, strategy, 'batch') : console.log('no minify strategy')))
	...
	spa: function (src, target, strategy) {
		...
		chain = chain.then(() => verify(src, target))
		chain = chain.then(() => buildify(this.build(src, target, options)))
		chain = chain.then(() => buildify(strategy ? this.minify(target, settings.options.spa, strategy, 'batch') : console.log('no minify strategy')))
		chain = chain.then(() => buildify(this.favicon(src, target, 'spa')))
	...
	build: function (src, target, options) {
		checkSrc(src)
		checkTgt(target)
		const buildify2 = (pvar) => new Promise((resolve, reject) => {
