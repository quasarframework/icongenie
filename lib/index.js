...
const checkSrc = async function (src) {
  if (image !== false) {
    ...
    if (!srcExists) {
      ...
      // process.exit(1)
      // throw 'Source image for icon-factory is not a png'
      throw '* [ERROR] Source image for icon-factory not found'
    } else {
      ...
      if (isPng(buffer) === true) {
        ...
        process.exit(1)
        // throw '* [ERROR] Source image for icon-factory is not a png'
      }
    }
  }
}
