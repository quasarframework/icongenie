...
let icongenie = exports.icongenie = {
  ...
  proton: async function(src, target, strategy, options) {
    const spinnerInterval = spinner()
    options = options || settings.options.proton
    await this.validate(src, target)
    progress('Building Proton icns and ico')
    await this.icns(src, target, options, strategy)
    progress('Building Proton icons')
    await this.build(src, target, options)
    if (strategy) {
      progress(`Minifying assets with ${strategy}`)
      await this.minify(target, options, strategy, 'batch')
  ...
  browserExt: async function(src, target, strategy, options) {
  const spinnerInterval = spinner()
  options = options || settings.options.browserExt
  await this.validate(src, target)
  ...
  kitchensink: async function(src, target, strategy) {
    ...
    await this.proton(src, path.join(target + '/src-electron/icons'), strategy)
