...
let iconfactory = (exports.iconfactory = {
  ...
  splash: function(src, target, options) {
    ...
    let feature = function() {
      return sharp(src) // make our source first then build promise chain
  ...
  icns: function(src, target) {
    return mkdirpAsync(`${target}/electron`).then(() => {
      sharp(src)
        .resize(256, 256)
        // .crop(sharp.strategy.centre) // deprecated
        .png()
  ...
  favicon: function(src, target, dest) {
    if (!dest) dest = 'extras'
    return mkdirpAsync(`${target}/${dest}`).then(() => {
      let image = sharp(src)
      return image
        .resize(32, 32)
        // .crop(sharp.strategy.centre) // deprecated
