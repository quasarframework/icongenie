...
const uniqueFolders = function (options) {
  ...
  for (let type in options) {
    if (options[type].folder !== undefined) {
...
let iconfactory = exports.iconfactory = {
  ...
  favicon: async function(src, target) {
    try {
      ...
      //  .toFile(`${target}/icons/icon-32x32.png`)

      // await sharpSrc.resize(16, 16).png()
      //  .toFile(`${target}/icons/icon-16x16.png`)

      let files = await Promise.all([
        readFile(`${target}/icons/favicon-16x16.png`),
        readFile(`${target}/icons/favicon-32x32.png`)
      ])
      let buf = await toIco(files)
      await writeFile(`${target}/icons/favicon.ico`, buf)
    } catch (err) {
      console.error(err)
    }
  },
  ...
  svg: async function(src, target, options) {
    try {
      ...
      await svgo.optimize(svg).then(res => {
        writeFile(`${target}/icons/safari-pinned-tab.svg`, res.data)
      })
