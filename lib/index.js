...
let icongenie = exports.icongenie = {
  ...
  favicon: async function(src, target) {
    try {
      ...
      let f32 = await sharpSrc.resize(32, 32).png().toBuffer()
        // .toFile(path.join(target, 'icons/32x32.png'))

      let f16 = await sharpSrc.resize(16, 16).png().toBuffer()
        // .toFile(path.join(target, 'icons/16x16.png'))

      let files = await Promise.all([ f32, f16 ])
      let buf = await toIco(files)
      ensureFileSync(path.join(target, '/icons/favicon.ico'))
      await writeFile(path.join(target, '/icons/favicon.ico'), buf)
