...
const extendCordova = async function () {
...
let iconfactory = exports.iconfactory = {
  ...
  cordova: async function(src, target, strategy, options) {
    ...
    await this.build(src, target, options, true)
    if (strategy) {
   ...
   * @param {boolean} background - use backgroudn color instead of transparency
   */
  build: async function(src, target, options, background) {
  ...
  svg: async function(src, target, options) {
    try {
      ...
      await svgo.optimize(svg).then(res => {
        writeFile(`${target}/safari-pinned-tab.svg`, res.data)
      })
