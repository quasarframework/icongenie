...
const paramsSchema = Joi.object({
  ...
  landscape: Joi.string().min(1),
  portrait: Joi.string().min(1),

  filter: Joi.string().valid(...generatorsList),
  quality: Joi.number().integer().min(1).max(12),
  color: Joi.string().pattern(/^#[0-9A-Fa-f]{3}([0-9A-Fa-f]{3})?$/),
  splashscreen: Joi.string().valid('pure', 'bg', 'overlay')
})

const assetsSchema = Joi.array().items({
  handler: Joi.string().required().valid(...generatorsList),
  name: Joi.string().required().min(1),
  folder: Joi.string().required().min(1),

  background: Joi.when('handler', { is: 'png', then: Joi.boolean() }),

  platform: Joi.when('handler', [
